let izumi = async (m, { conn, text }) => {
    ...
    try {
        ...
        const { result: x2 } = await (await fetch('https://izumiiiiiiii.dpdns.org/tools/upscale?imageUrl=' + encodeURIComponent(url))).json();

        // Result Upscale 2x Nya
        const { result } = await (await fetch('https://izumiiiiiiii.dpdns.org/tools/upscale?imageUrl=' + encodeURIComponent(x2.imageUrl))).json();
